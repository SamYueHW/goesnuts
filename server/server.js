...
app.use(cors({
 origin: ['https://enrichpos.com.au:3001','https://enrichpos.com.au','http://localhost:3000','http://45.113.94.105:3000',],
  // origin: ['https://enrichpos.com.au:3001','https://enrichpos.com.au','http://localhost:3000'],
...
app.post('/registerMember', async (req, res) => {
  ...
  try {
    ...
    if (firstName.length > 100 || middleName.length > 100 || lastName.length > 100 || email.length > 100 || contactNumber.length > 20 || address.length > 100 || suburb.length > 100 || state.length > 100 || postcode.length > 20 || associationName.length > 100 || associationId.length > 100) {
    ...
    const result = await executeDb(insertMemberQuery, [firstName, middleName, lastName, email, contactNumber, address, suburb, state, postcode, hashedPassword, associationName, associationId, storeId, 0]);
     // 将注册信息发送给某人
...
app.post('/customerRegister', async (req, res) => {
  ...
  const { firstName, middleName, lastName, email, phone, address, Suburb, Postcode, State, password, confirmPassword, storeId } = req.body;
  ...
  try {
    ...
    // Insert the new user
    const insertUserQuery = 'INSERT INTO customers (CustomerSurname,CustomerMiddleName, CustomerLastName, CustomerEmail, Password, StoreId, AssociationName, AssociationId, CustomerPhone, Address, Suburb, PostCode, State) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    const result = await executeDb(insertUserQuery, [firstName,middleName, lastName, email, hashedPassword,storeId, associationName, associationId, phone, address, Suburb, Postcode, State]);

    const newUserId = result.insertId;

    // Retrieve the new user
    const userQuery = 'SELECT * FROM customers WHERE CustomerId = ?';
    const user = await executeDb(userQuery, [newUserId], { fetchOne: true });

    // Generate JWT token
    const token = jwt.sign({ userId: user.CustomerId }, process.env.JWT_SECRET_KEY, { expiresIn: '10y' });

    // Remove password from user data before sending
    delete user.Password;

    if (req.body.applyForMembership) {
    ...
    res.json({ user, token, isApplyForMembership: req.body.applyForMembership });
...
app.post('/calculateShippingCost', async (req, res) => {
  try {  
    ...
    const storeId = await getStoreIdByUrl(storeUrl); 
    // tagSummary = {
    //   '11': { quantity: 2, weight: 240 },
    //   '12': { quantity: 1, weight: 1200 }
    // },
    ...
    console.log('total:', totalUnit, totalWeight);
    console.log('parcelPer:', weightPerParcel, unitPerParcel,numberOfParcels);
    ...
    const { Length, Width, Height } = chooseParcelResults;
...
//   }
// }这个是result 要乘以数量
  result.AUS_PARCEL_EXPRESS.total_cost = result.AUS_PARCEL_EXPRESS.total_cost * numberOfParcels;
  result.AUS_PARCEL_REGULAR.total_cost = result.AUS_PARCEL_REGULAR.total_cost * numberOfParcels;
    ...
    try {
      ...
      jwt.verify(token, process.env.JWT_SECRET_KEY, async(err, decoded) => {
        ...
        if (decoded.storeAdmin) {
          ...
          //two week ago
          const twoWeekAgo = moment().tz('Australia/Melbourne').subtract(14, 'days').format('YYYY-MM-DD 00:00:00');
            ...
            if (storeConfig.parcels.length > 0) {
              ...
              for (const parcel of storeConfig.parcels) {
                ...
                if (foundParcel.length > 0) {
                  ...
                  const updateParcelQuery = 'UPDATE parcelsetting SET Length = ?, Width = ?, Height = ?, Capacity = ? WHERE StoreId = ? AND ParcelId = ?';
...
app.get('/fetchTagsWithUnits', async (req, res) => {
  try {
    ...
    jwt.verify(token, process.env.JWT_SECRET_KEY, async (err, decoded) => {
      ...
      if (decoded.storeAdmin) {
        const storeId = decoded.storeId;
        const tagsQuery = 'SELECT * FROM item_tags WHERE StoreId = ?';
...
app.post('/updateCustomerAccount', authenticateJWT, async (req, res) => {
  ...
  const updateCustomerQuery = 'UPDATE customers SET CustomerSurname = ?, CustomerMiddleName = ?, CustomerLastName = ?, CustomerEmail = ?, CustomerPhone = ?, Address = ?, Suburb = ?, State = ?, PostCode = ?, Country = ?,  DeliveryAddress = ?, DeliverySuburb = ?, DeliveryState = ?, DeliveryPostCode = ?, DeliveryCountry = ? WHERE CustomerId = ?';
  await executeDb(updateCustomerQuery, [editedCustomer.CustomerSurname, editedCustomer.CustomerMiddleName, editedCustomer.CustomerLastName, editedCustomer.CustomerEmail, editedCustomer.CustomerPhone, editedCustomer.Address, editedCustomer.Suburb, editedCustomer.State, editedCustomer.PostCode, editedCustomer.Country,  editedCustomer.ShippingAddress, editedCustomer.ShippingSuburb, editedCustomer.ShippingState, editedCustomer.ShippingPostCode, editedCustomer.ShippingCountry, req.user.userId]);
  ...
  const customer = await executeDb(customerQuery, [ req.user.userId], { fetchOne: true });
...
async function sendOrderEmail(recipientEmail, subject, orderInfo) {
  ...
  // Paths to images
  const storeId = '246'; // Assuming a single store ID; modify as needed
  ...
  const itemsHtml = itemDetails.map(item => {
    ...
    let itemImageUrl = '';
    if (fs.existsSync(path.join(__dirname, `../public/images/${storeId}/stockItems/${item.stockId}.jpg`)) === false) {
      itemImageUrl =  `${serverUrl}/images/default-product-image.png`;
    } else {
      itemImageUrl = `${serverUrl}${item.imgUrl}`;
    }
...
app.use((err, req, res, next) => {
  ...
  const PORT = 5053; //外部是5052公开，转发入5053
  server.listen(PORT, () => {
